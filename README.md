...
`recall@10`/`NDCG@10` варианта `rrf+rerank+prox` просели ниже порога.

```bash
uv run habitus eval --check                              # дефолты: 0.29 / 0.30
uv run habitus eval --check --min-recall 0.25 --min-ndcg 0.20
```

Дефолты `--min-recall`/`--min-ndcg` — измеренное значение минус 0.05 (запас
