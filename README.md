...
### Завести партнёра и выпустить ключ

```bash
cd backend
go run ./cmd/partner create --slug gorod --name "Агентство «Город»"
go run ./cmd/partner key --slug gorod --scopes search:read,leads:read,webhooks:write
go run ./cmd/partner keys --slug gorod
go run ./cmd/partner revoke --prefix hab_live_k4m2rq7t
```

Секрет ключа показывается **один раз**: в базе лежит только его SHA-256.
Потерянный ключ не восстанавливается — выпускается новый, старый отзывается.

Ключи не выдаются через сам API намеренно: первый ключ выдавать было бы
нечем, а ручка «создай себе ключ» — это ручка «создай себе доступ».
