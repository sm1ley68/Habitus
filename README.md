...
- [Partner API (B2B)](#partner-api-b2b)
...
Публичный контур для интеграций — агентств, порталов, CRM. Живёт в том же
процессе, что и шлюз, но отдельной группой `/partner/v1`: у него другая
аутентификация (ключ вместо cookie-сессии), другой конверт ошибки, свои
лимиты и свои обещания по совместимости. Смешать его с `/api/v1` значит
однажды сломать интеграцию партнёра правкой, сделанной ради фронта.

**Документация для разработчиков — `http://localhost:8080/docs`.**
Машиночитаемый контракт — `http://localhost:8080/partner/v1/openapi.json`
(OpenAPI 3.1). Оба вшиты в бинарь: спецификация, способная разъехаться с
задеплоенным кодом, хуже, чем её отсутствие.

### Что отдаётся наружу

| Область | Ручки |
|---|---|
| Поиск | `POST /search`, `GET /searches/{id}`, `GET /searches/{id}/results` |
| Объекты | `GET /searches/{id}/properties/{obj}` (с досье), `POST .../ask`, `GET /properties/{obj}` |
| Город | `GET /geo/layers`, `GET /geo/listings` |
| Витрина | CRUD `/listings` + `/publish`, `/unpublish` |
| Заявки | `GET /leads` |
| События | CRUD `/webhooks`, `/test`, `/deliveries` |
| Служебные | `GET /me`, `GET /status`, `GET /openapi.json` |

Механика, общая для всех ручек: ключ `Authorization: Bearer hab_live_…` со
скоупами, курсорная пагинация, `Idempotency-Key` на POST, две раздельные
квоты (общая на минуту и на вызовы модели — в час) с заголовками
`RateLimit-*`, единый конверт ошибки с `type`/`code`/`param`/`request_id`.

### Как это устроено внутри

Партнёр — это служебная строка в `users` плюс строка в `partners`. Благодаря
этому весь уже написанный код кабинета продавца (`owner_listings`, `leads`)
работает для партнёра без единой правки, а заявка покупателя по объявлению
партнёра сама уезжает вебхуком в его CRM.

Поиск партнёра — самостоятельный ресурс (`partner_searches`), а не чат: у
интеграции чата нет, но контекст запроса нужен тот же — без разобранного
запроса «этот объект подходит на 84%» не на чем основать.

### Как выдаётся доступ

Только из терминала и только с админ-токеном. Ручки «создай себе ключ» в API
нет и не будет: это ручка «создай себе доступ».

Одноразовая настройка сервера:

```bash
openssl rand -hex 32                      # → PARTNER_KEY_PEPPER
HABITUS_ADMIN_TOKEN=… go run ./cmd/partner hash-token   # → PARTNER_ADMIN_TOKEN_HASH
```

Дальше, с админ-токеном в окружении:

```bash
cd backend
export HABITUS_ADMIN_TOKEN=…        # не флагом: флаг попадёт в историю оболочки
export HABITUS_OPERATOR=yarik       # иначе возьмётся пользователь ОС
...
Четыре рубежа, каждый закрывает свой сценарий:

| Рубеж | Что не даёт сделать |
|---|---|
| **Перец** (`PARTNER_KEY_PEPPER`) | Дамп или доступ к базе не даёт вписать себе рабочий ключ: хеш — `HMAC(перец, секрет)`, а перца в базе нет. Подобрать существующий offline тоже нельзя |
| **Админ-токен** | Случайную и «мимоходом» выдачу. В конфиге лежит только SHA-256 токена, поэтому чтение конфига его не выдаёт |
| **Одобрение** | Ключ партнёру в состоянии `pending`. «Завели» и «пустили в бой» — разные решения, и второе принимает человек |
| **Журнал** | Безымянную выдачу: каждое действие пишет оператора, основание и время в `partner_admin_log` |

Плюс два ограничения на самом ключе: **срок 180 дней** по умолчанию
(бессрочный — только явным `--forever`) и **список адресов**, с которых ключ
работает. Запрос с чужого адреса получает `403 ip_not_allowed`.

Секрет ключа показывается **один раз**. В базе лежит только его отпечаток —
восстановить нельзя ни нам, ни партнёру: выпускается новый, старый отзывается.

> За балансировщиком задайте `TRUSTED_PROXIES` и `PROXY_HEADER`, иначе
> allowlist проверяет адрес балансировщика, то есть не проверяет ничего.
...
| `PARTNER_API_ENABLED` | `true` | Рубильник контура `/partner/v1`. Выключенный не регистрирует ни одного маршрута — снаружи не видно, что он существует |
| `PARTNER_RATE_LIMIT_PER_MIN` | `120` | Общий потолок запросов партнёра, когда индивидуальный не задан в его строке |
| `PARTNER_LLM_PER_HOUR` | `60` | Отдельный потолок ручек, за которыми стоит вызов модели |
| `PARTNER_IDEMPOTENCY_TTL_HOURS` | `24` | Сколько помнить ответ на `Idempotency-Key` |
| `PARTNER_SEARCH_TTL_DAYS` | `30` | Срок жизни сохранённого поиска партнёра вместе с выдачей |
| `PARTNER_SWEEP_MINUTES` | `360` | Как часто чистить протухшие поиски и ключи идемпотентности |
| `PARTNER_WEBHOOK_TIMEOUT_S` | `10` | Таймаут одной доставки вебхука |
| `PARTNER_WEBHOOK_POLL_S` | `15` | Как часто диспетчер разбирает очередь доставок |
| `PARTNER_WEBHOOK_ALLOW_INSECURE` | `false` | Разрешить подписки на `http://` и локальные адреса. **Дыра SSRF** — только локальная разработка |
| `PUBLIC_BASE_URL` | `http://localhost:8080` | Из него собирается `doc_url` в конверте ошибки |
| `PARTNER_KEY_PEPPER` | — | **Обязателен** при включённом контуре, иначе шлюз не стартует. Перец для хеша ключа; в базе его нет. Смена обнуляет все выпущенные ключи |
| `PARTNER_ADMIN_TOKEN_HASH` | — | SHA-256 админ-токена. Пусто — выдача доступа выключена |
| `TRUSTED_PROXIES` | — | Прокси, чьему заголовку с адресом клиента можно верить |
| `PROXY_HEADER` | — | Заголовок с адресом клиента, например `X-Forwarded-For` |
