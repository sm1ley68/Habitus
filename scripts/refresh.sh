...
#
# ВАЖНО про гашение: снимок в 400 объявлений покрывает малую долю активной базы,
# поэтому deactivate_missing его отвергает (SNAPSHOT_COVERAGE_MIN) — иначе один
# щадящий цикл гасил бы всё, что собрали предыдущие. Значит снятое с продажи в
# щадящем режиме НЕ гаснет. Чтобы база не накапливала покойников, один запуск в
# сутки должен быть полным: HABITUS_MAX_OFFERS=6000 (см. docs/cian-parser.md,
# раздел про планировщик). Полный обход идёт по тем же фильтрам, но глубже, и
# его стоит ставить в ночное окно.
