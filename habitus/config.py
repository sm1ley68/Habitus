...
class Settings(BaseSettings):
    ...
    # нему 60 выглядит лучшей точкой. Пул 30 совпадает с result_max_n и сокращает
    # число пар кросс-энкодера на 25% относительно прежнего значения 40.
    ...
    # result_max_n. Это про ДЕФОЛТНЫЙ путь: явный SearchRequest.top_n (контракт
    # допускает до 50) поднимает пул до запрошенного числа — см. run_search,
    # иначе потолок пула молча урезал бы выдачу ниже обещанного контрактом.
    rerank_pool_n: int = 30
    # На CPU сохраняем тот же пул 30: это не обрезает выдачу ниже result_max_n.
    # Замер docs/notes/rerank-pool-2026-08-18.md подтверждает, что качество
    # остаётся стабильным в исследованном диапазоне; основной выигрыш даёт
    # сокращение длины последовательности ниже.
    rerank_pool_n_cpu: int = 30
    ...
    #   256 — recall@10 0.65, NDCG 0.66, 23.3 с
    #   192 —              не мерено,             17.3 с
    # Для сбалансированного CPU-профиля выбрано 128: прямого замера этой точки
    # пока нет, но структурные факты занимают максимум 85 токенов и помещаются
    # целиком, поэтому сокращается прежде всего свободное описание.
    ...
    #: Вес суммарной удалённости объекта от точек, которые семья реально
    #: назвала (работа, школа, секция). Сигнал, а не фильтр: цель, которую не
    #: нашёл геокодер, в него не входит, а отсутствие geom у объявления —
    #: не штраф, а неучастие. Порядок величины тот же, что у ориентации:
    #: домохозяйство двигает выдачу, но не подменяет семантику запроса.
    household_weight: float = 0.2
