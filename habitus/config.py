...
class Settings(BaseSettings):
    ...
    #: НЕ подобран по данным — заглушка, см. docs/notes/techdebt-household-weight.md.
    #: Развёртка по d-серии его не даёт ни на каком эталоне: эталон строится той
    #: же метрикой, которой канал упорядочивает выборку, и меряет согласие сигнала
    #: с самим собой. Закрывается только разметкой человеком.
