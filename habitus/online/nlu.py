...
- Член домохозяйства — тот, у кого есть РЕГУЛЯРНЫЕ поездки: человек или \
животное. Заводи его в household, когда он сам явно назван и названо либо \
конкретное место поездки («Лицей 239»), либо КЛАСС мест («в школу», «в парк»). \
id — короткий латинский slug, label — исходное обозначение («Сын», «Жена», \
«Собака»).
- Класс мест пиши в to_label одним словом-категорией («школа», «парк», \
«метро») и не выдумывай названия: точку такого места система намеренно не \
ставит, а меряет пешую доступность всей категории у каждого объявления.
- Собака — член домохозяйства с регулярной поездкой «выгул»: to_kind "park", \
mode "walk", to_label "парк". Зона выгула и парк для подбора района — одно и \
то же место. У кошки и прочих домашних животных регулярных поездок нет — \
household под них не заводи.
...
- Бюджет времени на метро («40 минут до работы на метро», «без машины, час до \
центра») в ParsedQuery положить некуда: это не household.legs.depart/arrive \
(там только часы:минуты) и не поле point (оно есть только в SearchRequest, \
задаётся вызывающей стороной, а не разбором текста — «метро» в запросе не \
повод его синтезировать). Само число минут парсер намеренно отбрасывает: не \
пишет в semantic_text и не выдумывает под него household-персону. Если при \
этом человек и место названы явно (см. правило выше), заведи для него обычную \
household-поездку с mode "metro"; если явно названного человека нет — \
household не трогай вовсе, даже если место названо.
- «без машины» меняет mode ТОЛЬКО у уже названных поездок, у которых режим \
ещё не назван явно; уже явно названный режим («идёт пешком» → mode "walk") \
не переопределяй.
- Не путай два разных намерения на слове «метро»: «5 минут пешком до метро» — \
geo с kind "metro" (пешая доступность станции), а «40 минут на метро» — \
бюджет поездки конкретного человека (mode "metro" у его household-ноги); они \
...
Запрос: «двушка на севере москвы в тихом районе, живём с собакой»
→ {"rooms": [2], "area": "север", "noise_max": "low", "semantic_text": "", \
"lang": "ru", "household": [{"id": "dog", "label": "Собака", "legs": \
[{"to_label": "парк", "to_kind": "park", "mode": "walk"}]}]}

Запрос: «однушка недорого, у меня кошка»
→ {"rooms": [1], "semantic_text": "", "lang": "ru"}
...
Запрос: «сын, без машины, минут 40 до лицея 239 на метро»
→ {"household": [{"id": "son", "label": "Сын", "legs": \
[{"to_label": "Лицей 239, Москва", "to_kind": "school", "mode": "metro"}]}]}

Запрос: «двушка, без машины, до Сити не больше 40 минут на метро»
→ {"rooms": [2], "semantic_text": "", "lang": "ru"}
"""
