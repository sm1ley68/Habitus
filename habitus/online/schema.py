...
class RouteLeg(BaseModel):
    ...
    to_kind: DestinationKind
    mode: TravelMode
    # Часы поездки — ТОЛЬКО когда пользователь их назвал. NLU запрещено
    # выдумывать depart/arrive (см. промпт в nlu.py), поэтому у большинства ног
    # времени нет вовсе. Раньше поля были обязательными, и нога без времени
    # выбрасывалась целиком — вместе с ней пропадал весь блок «Суточный ритм
    # семьи» на каноническом запросе вида «ребёнок в школу пешком, супруг в
    # Сити», где ни одного часа не названо. Длительность (minutes) известна
    # всегда: она посчитана, а не названа.
    depart: str | None = None
    arrive: str | None = None
    minutes: int = Field(ge=0)
    # None — безопасность маршрута не измеряли. Раньше поле было обязательным и
    # заполнялось константой по режиму («метро — safe, остальное — caution»),
