...
from habitus.geo.metro_access import (
    MAX_ENTRY_WALK_METRES as _MAX_ENTRY_WALK_METRES,
)
...
#: Потолок пешего плеча до входа/выхода (R63, фикс-раунд 1). Само число и
#: его обоснование переехали в нижний слой — habitus/geo/metro_access.py,
#: рядом со straight_walk_seconds, которым оно считается (R92, сквозное
#: ревью ветки): офлайновый писатель строк доступа обязан отбрасывать
#: платформы по ТОМУ ЖЕ потолку, что и рантайм, иначе фильтр по времени и
#: досье одного и того же объекта отвечают по-разному. Имя оставлено здесь
#: реэкспортом: на него завязаны и вызовы ниже, и тесты.
MAX_ENTRY_WALK_METRES = _MAX_ENTRY_WALK_METRES
...
#: R90 (сквозное ревью ветки): доля объявлений города, о которых предикат не
#: может судить и которые поэтому молча выпадают из выдачи. Судить он может
#: только о тех, у кого есть строка доступа к платформе, ДОСТИЖИМОЙ от цели
#: по графу; платформа в другой компоненте разорванного графа даёт не
#: «дольше N минут», а «неизвестно» — а SQL-фильтр этой разницы не знает и
#: одинаково выбрасывает обоих.
#:
#: Порог — инженерное решение, а не измерение: фильтр, который не в
#: состоянии оценить каждое десятое объявление города, уже не сужает выдачу
#: по времени, а режет её по случайному признаку «на какой линии дом».
#: Такой лучше снять целиком и сказать об этом, чем применить молча.
#: Замеры на живых данных, ради которых порог и появился:
#:   msk — 2 компоненты (395 + 7 станций: Бутовская линия связана с
#:         Новоясеневской только пересадкой РАЗНОИМЁННЫХ станций, которой в
#:         data/metro/msk.json нет), не оценивается 43 объявления из 6738 —
#:         0.6%, фильтр применяется и несёт заметку про эти 43;
#:   spb — 6 компонент (19/16/15/12/9/2 станции), 0 строк в metro_transfer:
#:         петербургские узлы называются на каждой линии по-своему
#:         («Невский проспект» ↔ «Гостиный двор»), а курировать их — это
#:         выдумывать факты о городе, что проектом запрещено (та же логика,
#:         что в R27). Межлинейная маршрутизация там невозможна, и фильтр
#:         снимается целиком.
MAX_UNJUDGED_SHARE = 0.10


def _coverage(conn: psycopg.Connection, city: str,
              reachable: list[int]) -> tuple[int, int]:
    """(судимые, всего) — объявления города со строками пешего доступа.

    «Судимое» — то, у которого хотя бы одна платформа доступа достижима от
    цели по графу. Популяция та же, что у поиска (`is_active = TRUE`,
    habitus/online/retrieval.py), иначе доля считалась бы по одному
    множеству, а фильтр применялся к другому.
    """
    row = conn.execute("""
        SELECT count(*) FILTER (WHERE judged), count(*)
        FROM (SELECT bool_or(a.station_id = ANY(%s::bigint[])) AS judged
              FROM listing_metro_access a
              JOIN listings l ON l.external_id = a.external_id
              WHERE l.city = %s AND l.is_active
              GROUP BY a.external_id) t;""",
        (reachable, city)).fetchone()
    return (row[0] or 0, row[1] or 0) if row else (0, 0)


def metro_predicate_with_note(
        conn: psycopg.Connection, city: str, lon: float, lat: float,
        minutes: int) -> tuple[tuple[str, tuple] | None, str | None]:
    """То же, что `metro_predicate()`, плюс причина деградации словами.

    Возвращает (предикат, заметка). Предикат `None` — фильтровать нечем,
    ограничение не накладывается вовсе (синтетический ноль запрещён, молча
    обнулять выдачу нельзя). Заметка не `None` — вызывающий ОБЯЗАН показать
    её пользователю: `pipeline.py` (шаг 4.5) сознательно не пускает
    point-предикаты в `constraint_diagnostics`, поэтому `relaxed` —
    единственный канал, которым про снятый или неполный фильтр вообще можно
    узнать (R70).

    Заметка приходит и вместе с НЕ-`None` предикатом: фильтр применён, но
    часть города он оценить не смог (разорванный граф, см.
    MAX_UNJUDGED_SHARE) — это не деградация до отсутствия, но и не молчание.

    Причины `None`, все четыре — «предикат не накладывается», а не «всё
    отфильтровано в ноль»:
      ...
      2. у цели нет ни одной платформы в пределах MAX_ENTRY_WALK_METRES —
         `nearest_stations` может вернуть пустой словарь (R68), и это не
         «нигде не доехать за N минут», а «метро тут вообще не при чём»;
      3. R91: по городу нет НИ ОДНОЙ строки пешего доступа. Путь достижим:
         в `habitus/cli.py` сборка графа коммитится, а упавший
         `refresh_listing_metro_access` откатывается — каскадно вычищенные
         строки доступа тогда не восстанавливаются. Раньше это было
         неотличимо от «ничего не укладывается в N минут»: обе ситуации
         давали пустую выдачу без единой заметки;
      4. R90: граф города разорван настолько, что о доле объявлений больше
         MAX_UNJUDGED_SHARE предикат судить не может.

    Время до каждой достижимой платформы берётся ИЗ ТОГО ЖЕ обхода
    `times_from`, что и разбивка маршрута в `door_to_door()` (Задача 11) —
    единственный способ гарантировать, что число фильтра и показанный
    пользователю маршрут не разойдутся (см. докстроку
    `MetroGraph.times_from`). Никакой второй формулы времени здесь нет и не
    должно появляться.
    """
    graph = load_graph(conn, city)
    if graph is None:
        return None, "метро: графа города нет — фильтр по времени не наложен"
    targets = nearest_stations(conn, city, lon, lat)
    if not targets:
        return None, ("метро: у точки нет платформ в пешей доступности — "
                      "фильтр по времени не наложен")
    times = graph.times_from(targets)
    if not times:
        return None, ("метро: у точки нет платформ в пешей доступности — "
                      "фильтр по времени не наложен")

    judged, total = _coverage(conn, city, list(times.keys()))
    if total == 0:
        # R91: строк доступа по городу нет вовсе — фильтровать не по чему.
        # Отличается от «ничего не укладывается в бюджет» именно заметкой.
        return None, ("метро: пешие плечи до платформ по городу не рассчитаны — "
                      "фильтр по времени не наложен")
    unjudged = total - judged
    if judged == 0 or unjudged > total * MAX_UNJUDGED_SHARE:
        # R90: цель достижима лишь для меньшинства объявлений — остальные
        # предикат выбросил бы не как «далеко», а как «неизвестно». Лучше
        # отсутствие фильтра с заметкой, чем тихо неверно суженная выдача.
        # Формулировки заметок намеренно без счётного существительного
        # после числа: они уходят пользователю как есть, а «43 объявлений»
        # против «43 объявления» — вопрос склонения, которого на этом слое
        # нет чем решить (pluralRu живёт на фронте, frontend/lib/format.ts).
        return None, (
            f"метро: граф города разорван (нет данных о пересадках между "
            f"линиями) — время до цели неизвестно для части объявлений "
            f"({unjudged} из {total}), фильтр по времени не наложен")
    ...
    note = None
    if unjudged:
        note = (f"метро: часть объявлений ({unjudged} из {total}) не оценена "
                f"— их платформы не связаны с целью в графе (нет данных о "
                f"пересадках)")
    return (sql, tuple(params)), note
...
def metro_predicate(conn: psycopg.Connection, city: str, lon: float, lat: float,
    ...
    подхватывает ограничение как обычную клаузу.

    Тонкая обёртка над `metro_predicate_with_note()`, которая ВЫБРАСЫВАЕТ
    причину деградации. Вызывающему, который показывает выдачу пользователю
    (оркестратор), нужна именно версия с заметкой: `None` без объяснения —
    это молча снятый фильтр, ровно то, что запрещает R70/R90.
    """
    return metro_predicate_with_note(conn, city, lon, lat, minutes)[0]
