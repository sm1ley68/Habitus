...
def metro_predicate(conn: psycopg.Connection, city: str, lon: float, lat: float,
    ...
    R70 (фикс-раунд 1): вопреки более раннему черновику этого комментария,
    `constraint_diagnostics` этот предикат НЕ видит — `pipeline.py` (шаг 4.5)
    сознательно не передаёт point-предикаты в диагностику (там это
    зафиксировано отдельным комментарием). Единственный канал, которым
    вызывающий (orchestrator) обязан сообщить о сработавшем `None` —
    заметка в `relaxed`, а не диагностика.
